        ...
        if updater.update_ready == True:
            ...
            if updater.verbose:
                if res == 0:
                    ...
                    print("Updater returned " + str(res) + ", error occured")
            ...
            try:
                ...
                if updater.verbose:
                    if res == 0:
                        ...
                        print(
                            "Updater returned " + str(res) + ", error occured")
            except:
...
def update_notice_box_ui(self, context):
    saved_state = updater.json
    if updater.auto_reload_post_update == False:
        if "just_updated" in saved_state and saved_state[
            "just_updated"] == True:
            layout = self.layout
            box = layout.box()
...
def update_settings_ui(self, context):
    ...
    if updater.error != None:
        ...
        col.operator("wm.url_open",
                     "Download " + str(
                         updater.update_version)).url = updater.website
...
def check(bl_info):
    ...
    0, 0, 0)  # min install (>=) will install this and higher
    # updater.version_min_update = None  # if not wanting to define a min
    updater.version_max_update = (
    9, 9, 9)  # max install (<) will install strictly anything lower
