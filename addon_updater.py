...
class Singleton_updater(object):
    ...
    def set_check_interval(self, enable=False, months=0, days=14, hours=0,
                           minutes=0):
    ...
    def get_tags(self):
        ...
        if self._tags == None:
            ...
            if self.verbose: print(
                "No releases or tags found on this repository")
            ...
            if self.verbose: print(
                "No versions found within compatible version range")
        else:
            self._tag_latest = self._tags[0]
    ...
    def get_api(self, url):
        ...
        get = None
        get = self.get_api_raw(
            url)  # this can fail by self-created error raising
        if get != None:
    ...
    def upack_staged_zip(self):
        ...
        origpath = os.path.dirname(
            __file__)  # verify, is __file__ always valid?
    ...
    def check_for_update(self, now=False):
        ...
        if self._update_ready != None and now == False:
            return (
            self._update_ready, self._update_version, self._update_link)
